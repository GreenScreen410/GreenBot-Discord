...
export default {
  ...
  run: async (client: Client, interaction: any) => {
    ...
    if (!queue.playing) {
      track.playlist ? queue.addTracks(track.playlist[0]) : queue.play(track.tracks[0]);
      return await interaction.followUp({ embeds: [embed] });
    } else if (queue.playing) {
      track.playlist ? queue.addTracks(track.playlist[0]) : queue.addTrack(track.tracks[0]);
      return await interaction.followUp({ embeds: [embed] });
    }
  },
};
